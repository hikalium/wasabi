...
impl TcpSocket {
    ...
    pub fn debug_summary(
        &self,
    ) -> Option<(TcpSocketState, usize, usize)> {
    ...
    pub fn handle_rx(
        ...
        let in_tcp = TcpPacket::copy_from_slice(
            &frame[..size_of::<TcpPacket>()],
        )
        .ok()?;
        ...
        let data_end =
            size_of::<IpV4Packet>() + tcp_total.min(frame.len() - size_of::<IpV4Packet>());
        ...
        match prev_state {
            ...
            TcpSocketState::SynReceived => {
                if !in_tcp.is_ack()
                    || in_tcp.ack_num() != inner.my_next_seq
                {
                    return None;
                }
            ...
            TcpSocketState::Established => {
                ...
                if in_tcp.is_ack() && inner.unacked_len > 0 {
                    ...
                    let acked = in_tcp
                        .ack_num()
                        .wrapping_sub(first_unacked_seq) as usize;
...
fn build_segment(
    ...
    let csum =
        tcp_segment_checksum(&bytes[segment_off..], our_ip, peer_ip);
...
mod tests {
    ...
    fn handle_rx_listen_to_syn_received() {
        ...
        let syn = build_client_segment(
            PEER_MAC, PEER_IP, 12345,
            OUR_MAC, OUR_IP, 23,
            5000, None, true, false, &[],
        );
        let reply = sock.handle_rx(&syn, OUR_MAC, OUR_IP, Duration::ZERO).unwrap();
    ...
    fn handle_rx_synreceived_to_established() {
        ...
        let synack = sock.handle_rx(&syn, OUR_MAC, OUR_IP, Duration::ZERO).unwrap();
        ...
        let ack = build_client_segment(
            PEER_MAC, PEER_IP, 12345,
            OUR_MAC, OUR_IP, 23,
            5001, Some(our_seq_plus_1), false, false, &[],
        );
    ...
    fn handle_rx_data_acks_and_buffers() {
        ...
        let seg = build_client_segment(
            PEER_MAC, PEER_IP, 12345,
            OUR_MAC, OUR_IP, 23,
            client_seq, Some(0), false, false, data,
        );
        let reply = sock.handle_rx(&seg, OUR_MAC, OUR_IP, Duration::ZERO).unwrap();
        let r = parse_reply(&reply);
        assert!(r.is_ack());
    ...
    fn poll_tx_keeps_bytes_until_acked() {
        ...
        let ack = build_client_segment(
            PEER_MAC, PEER_IP, 12345,
            OUR_MAC, OUR_IP, 23,
            5001, Some(first_seq.wrapping_add(5)), false, false, &[],
        );
    ...
    fn send_buffer_drains_across_many_poll_tx_acks() {
        ...
        for _ in 0..5 {
            ...
            let ack = build_client_segment(
                PEER_MAC, PEER_IP, 12345,
                OUR_MAC, OUR_IP, 23,
                5001, Some(p.seq_num().wrapping_add(payload_len)),
                false, false, &[],
            );
    ...
    fn handle_rx_fin_to_lastack_then_close() {
        ...
        let fin = build_client_segment(
            PEER_MAC, PEER_IP, 12345,
            OUR_MAC, OUR_IP, 23,
            client_seq, Some(0), false, true, &[],
        );
        let reply = sock.handle_rx(&fin, OUR_MAC, OUR_IP, Duration::ZERO).unwrap();
        let r = parse_reply(&reply);
